...
export class EthereumERC20EventTransferService {
  ...
  async main() {
    ...
    tokens.forEach(({ symbol, contract_address }) => {
      debug(`start sync transfer events (symbol: ${symbol})`);
      this.syncTransferEvents(contract_address);
    });
    debug(`start sync transfer events (token count: ${tokens.length})`);
  }
  ...
  async syncTransferEventsFromBlockNumber(contractAddress: string, blockNumber: number) {
    try {
      ...
      if (logs.length > 0) {
        ...
        debug(`sync transfer events (contract: ${contractAddress}, block: ${blockNumber}, event count: ${eventEntities.length}) success 🎉`);
      }
    } catch (e) {
      debug(`sync transfer events (contract: ${contractAddress}, block: ${blockNumber}) error:`, e.message);
    }
